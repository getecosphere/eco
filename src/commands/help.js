...
Current command groups:
  ...
  dashboard   Live summary of every managed estate on a target CT (ports,
             binaries, PM2 state) read from the estate registry + PM2
  show        Show the current service URLs/ports from ecosystem.config.js
  repos       List known repos from eco/repos.json
             Add current git repo with: eco repos add
  ...
  clearstarterproject Remove the placeholder starter runtime files from a
             <project>_composition repository, keeping the service contract
             files so a real frontend/backend can replace the starter
  compose     Add a repo to an existing ecompose.yml
             "eco compose add" with no args offers an arrow-key checklist of
             eco/repos.json repos not yet composed into this estate (same
             UX as startproject, including auto-selecting dependencies).
...
Examples:
  ...
  eco startproject .
  eco adopt
  eco adopt chronic/chronic_bootstrap
  eco compose add
...
GitHub automation env:
  GITHUB_TOKEN   GitHub token with repository webhook write access
  ECO_GITHUB_API_KEY
                 GitHub token used by startproject to create and push the
                 private <project>_bootstrap and <project>_composition repos
`);
}
