...
# service. The registry lives per machine -- ~/.eco/registry.db in dev,
# /etc/eco/registry.db on a root CT -- and scopes rows by hostname so a
# shared machine (multiple estates on one CT, or the dev laptop) has one
# collision-free port namespace.
...
# runs as part of the full up.js pipeline, and run_test_gates_before_deploy's
# copy of this same logic only fires when ECO_RUN_TESTS_BEFORE_DEPLOY is set
# (the webhook redeploy path). Any manual/partial recovery step in between
...
# Proxmox CT used as a personal remote dev machine, accessed over its
# LAN/Tailscale-routed IP. Next.js has no CLI flag or env var for this --
...
# ─── Test-gated deploy: a Rust service whose integration tests fail must
# not be deployed ───────────────────────────────────────────────────────────
#
# Only runs when explicitly requested (ECO_RUN_TESTS_BEFORE_DEPLOY=1) --
# `eco configure` runs constantly during local dev and interactive
# workspace setup, and a full `cargo test` sweep on every one of those
# would be a real slowdown for no benefit; this is meant to run exactly
# once, as part of the actual deploy pipeline (see
# buildDeployReceiverFiles's generated redeploy.sh in up.js), which is the
# only caller that sets the env var.
#
# Writes one PM2 app name per line to "$PM2_DIR/.eco-test-failures" for
# every rust service whose `cargo test` failed (truncated fresh each run;
# empty/absent means nothing failed). redeploy.sh reads that file to
# restart everything except the failed services, so a broken domain keeps
# running its last-good build instead of being torn down or blocking the
# rest of the estate's deploy.
run_test_gates_before_deploy() {
  if ! is_truthy "${ECO_RUN_TESTS_BEFORE_DEPLOY:-}"; then
    return 0
  fi

  local failures_file="$PM2_DIR/.eco-test-failures"
  : > "$failures_file"

  local i any_rust=false
  for i in "${!svc_name[@]}"; do
    [[ "${svc_type[$i]}" == "rust" ]] && any_rust=true
  done
  if [[ "$any_rust" == false ]]; then
    return 0
  fi

  # These tests hit a real local MongoDB. In the deploy pipeline this
  # function runs from redeploy.sh's "regenerate estate configuration"
  # step, which happens *before* the later "bootstrap data services" step
  # that normally restarts mongod -- so mongod isn't guaranteed to be up
  # yet here. Ensure it ourselves rather than risk every test failing on a
  # connection error and blocking a deploy for a reason that has nothing
  # to do with the actual code change.
  if command -v systemctl >/dev/null 2>&1; then
  ...
  for i in "${!svc_name[@]}"; do
    [[ "${svc_type[$i]}" != "rust" ]] && continue
    local name="${svc_name[$i]}" dir="${svc_dir[$i]}" app_name="${PROJECT_NAME}-${svc_name[$i]}"

    echo ""
    echo -e "${BOLD}Running tests: ${name}${RESET}"
    if ! command -v cargo >/dev/null 2>&1; then
      echo -e "  ${YELLOW}⚠${RESET} cargo not found — cannot gate ${name}, deploying without test verification"
      continue
    fi

    # Piping through sed for indentation would otherwise hide cargo's real
    # exit code behind sed's (always 0) -- capture it explicitly via
    # PIPESTATUS rather than relying on `set -o pipefail` being on.
    local test_exit=0
    (cd "$dir" && cargo test --quiet 2>&1 | sed "s/^/  /"; exit "${PIPESTATUS[0]}") || test_exit=$?

    if [[ "$test_exit" -eq 0 ]]; then
      echo -e "  ${GREEN}✓${RESET} ${name} — tests passed"
    else
      echo -e "  ${RED}✗${RESET} ${name} — tests FAILED, this service will not be deployed"
      echo "$app_name" >> "$failures_file"
    fi
  done

  if [[ -s "$failures_file" ]]; then
    echo ""
    echo -e "${RED}Test-gated deploy: the following services failed and will keep running their previous build:${RESET}"
    sed 's/^/  - /' "$failures_file"
  fi
}
...
VITESTART
      ...
      # Production builds are made before this config is generated (locally
      # or on ECO_RUST_DEDICATED_BUILDER). Prefer that artifact; starting
      # `cargo run` under every PM2 service recompiles concurrently and
      # defeats the shared workspace/build lock.
      local pkg_bin=""
    ...
    if [[ "$type" == "rust" ]] && is_prod_mode; then
      runtime_env_extra+=', RUSTUP_HOME: "/usr/local/rustup", CARGO_HOME: "/usr/local/cargo", PATH: "/usr/local/cargo/bin:" + process.env.PATH, RUSTC_WRAPPER: "/usr/local/bin/sccache", SCCACHE_DIR: "/usr/local/sccache-cache"'
    fi
