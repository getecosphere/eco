...
- `eco lxs add` also reads an LXS `compose:` recipe. A recipe can declare the
  service name, port, secret grants, safe non-secret defaults, Gateway routes,
  and applicable estate identity defaults. Eco writes these into
  `ecompose.yml` and adds the service to the main estate automatically.
  Existing user choices are preserved; run `eco lxs setup <name> --force` only
  for an intentional security migration.
...
compose:
  service:
    name: <name>-backend
    port: 4200
    grants: { secrets: [JWT_SECRET] }
    config: {}                 # non-secret, non-managed values only
    access:
      routes:
        - { path: /api/<name>/*, level: auth, cookie: eco_token }
runtime:
...
`compose.service.access.routes` is the same deny-by-default Gateway contract
as `services.<name>.access.routes`; every browser/API route required by the
LXS must be present. Never put a secret in `compose.config`: secrets are named
in `compose.service.grants.secrets` and generated by Eco. A domain that owns a
default identity policy may declare `compose.auth.email_verification.enabled`;
Eco makes it visible in the estate's top-level `auth:` block.
