...
fn ensure_builder(needs: &str) -> Result<(), String> {
    if builder_is_host() {
...
fn ensure_eco_builder_vm(needs: &str) -> Result<(), String> {
    if builder_available() {
        return Ok(());
    }
...
pub fn run_up_remote(args: &[String]) -> Result<(), String> {
    ...
    let mut builder_needs: Vec<&str> = Vec::new();
    if !rust_targets.is_empty() {
        builder_needs.push("rust");
    }
    if !frontend_targets.is_empty() {
        builder_needs.push("node");
    }
    if !builder_needs.is_empty() {
        ensure_builder(&builder_needs.join(","))?;
    }
...
fn install_lxs_services_local(
    ...
    for service in lxs_services {
        ...
        let start_sh = format!(
            "#!/bin/bash\nset -euo pipefail\ncd \"$(dirname \"$0\")\"\n# PM2 assigns the runtime port. `.env` is generated from contracts and may\n# contain an intentionally blank SERVER_PORT, so never let it erase PM2.\npm2_server_port=\"${{SERVER_PORT:-}}\"\nif [[ -f .env ]]; then set -a; source .env; set +a; fi\n[[ -n \"$pm2_server_port\" ]] && export SERVER_PORT=\"$pm2_server_port\"\n: \"${{SERVER_PORT:?SERVER_PORT not set}}\"\n# Astro/Node standalone servers read PORT, while Rust LXS use SERVER_PORT.\n# Export both so a mixed estate always receives the PM2-assigned port.\n[[ -n \"${{PORT:-}}\" ]] || export PORT=\"$SERVER_PORT\"\n[[ -n \"${{MONGODB_URI:-}}\" ]] || export MONGODB_URI=\"mongodb://localhost:27017/{db_name}_{project}\"\n[[ -n \"${{REDIS_URL:-}}\" ]] || export REDIS_URL=\"redis://127.0.0.1:6379\"\n[[ -n \"${{DATABASE_URL:-}}\" ]] || export DATABASE_URL=\"\"\nexec ./bin/{installed_name}\n",
            db_name = db_name,
