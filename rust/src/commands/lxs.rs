...
pub fn validate_grants_against_contract(
    ...
    // Only *unmanaged* secrets must be granted: managed keys (shared JWT, the
    // managed DB URI, the allocated port) are generated by Eco itself, so an
    // estate never declares them as grants.
    let mut required_secret: Vec<String> = fields
        .iter()
        .filter(|(_, f)| f.required && is_secret(f) && f.managed.is_empty())
        .map(|(k, _)| k.clone())
        .collect();
...
mod tests {
    ...
    fn secret_contract() -> LxsManifest {
        let mut fields = HashMap::new();
        // Managed required secret — Eco generates it, so it must NOT be
        // required as an estate grant (this is the gateway JWT_SECRET case).
        ...
        fields.insert("SERVER_PORT".to_string(), field("int", "", false, "port"));
        // Unmanaged required secret — the estate must grant it.
        let mut stripe = field("secret", "", true, "");
        stripe.required = true;
        fields.insert("STRIPE_KEY".to_string(), stripe);
    ...
    fn grants_cover_required_secret_and_reject_undeclared() {
        ...
        // STRIPE_KEY is required+unmanaged; MONGODB_URI is declared but not a
        // secret. JWT_SECRET is managed, so it needs no grant.
        assert!(validate_grants_against_contract(
            "auth-backend",
            &["STRIPE_KEY".to_string(), "MONGODB_URI".to_string()],
            &m
        )
        .is_ok());
        // A managed secret must not be demanded as a grant (gateway case).
        assert!(validate_grants_against_contract("auth-backend", &["STRIPE_KEY".to_string()], &m).is_ok());
        // Missing the required unmanaged secret is rejected.
        ...
        let err = validate_grants_against_contract(
            "auth-backend",
            &["STRIPE_KEY".to_string(), "NOT_IN_CONTRACT".to_string()],
            &m,
        )
        .unwrap_err();
