...
# later points PM2 at. A deployed estate runs inside a container, so this
# needs no explicit signal: it just detects the environment it's in.
detect_deploy_mode() {
...
Supported runtime tokens:
  ...
  golang
  rust
  python  (or python@3.11 — Django/Flask services)
  dotnet  (or dotnet@8 — ASP.NET Core services)
  git
  openssh-client
  curl
  jq
...
ensure_brew() {
  ...
  # Homebrew may already be installed but not on this shell's PATH (Apple
  # Silicon installs to /opt/homebrew, Intel to /usr/local). Use it instead of
  # re-installing (which would fail trying to sudo without a TTY).
  if [[ -x /opt/homebrew/bin/brew ]]; then
    export PATH="/opt/homebrew/bin:$PATH"
    ok "Using Homebrew at /opt/homebrew/bin/brew"
    return
  fi
  if [[ -x /usr/local/bin/brew ]]; then
    export PATH="/usr/local/bin:$PATH"
    ok "Using Homebrew at /usr/local/bin/brew"
    return
  fi
...
OVERRIDE
    ...
    dotnet|dotnet@*)
      if need_cmd dotnet; then
        ok "dotnet already installed: $(dotnet --version 2>&1)"
      else
        apt_install dotnet-sdk-8.0 2>/dev/null || {
          log "Installing .NET SDK 8 via the official script..."
          curl -fsSL https://dot.net/v1/dotnet-install.sh | $SUDO bash /dev/stdin --channel 8.0 --install-dir /usr/share/dotnet
