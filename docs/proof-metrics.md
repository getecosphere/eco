...
### 3.2 Remote CT (`eco up --remote`)

Each project built locally (cross-compile / npm / mvn / bun) and shipped as
executable-only artifacts to the shared app CT. Auth verified inside the CT.
...
`eco up --remote`) → Cloudflare Tunnel → proxy CT → app CT. The **whole app is
...
3. **Density.** All 9 estates (9 app services + 9 auth binaries) run on a single
   shared app CT alongside the existing production estates — host-native LXC,
...
- Remote host: private control-plane host running the eco-agent binary
  (`eco serve --port 8790`); infrastructure details live in the private repo.
- Shared application CT with the other estates (apindo/assessment/crm/
  getecosphere/etc); Cloudflare tunnels; zone `getecosphere.com`.
- LXS registry: `getecosphere/lxs-registry` (local mirror at `~/ar-rahman/lxs/registry`).

## 7. Binary & storage statistics (measured on the shared app CT)
...
- All 9 estates (≈744 MB, 18 systemd units) run on **one shared app CT**
...
Verified live on the shared app CT: register + login OK, root `/` 200.
...
eco up --remote     # systemd runs the shipped binaries
